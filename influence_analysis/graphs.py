...
SnapData: TypeAlias = tuple[int, np.ndarray]
...
class GraphGenerator:
    ...
    def read_snap_txt(data_file: Path, separator: str = '\t') -> SnapData:
        ...
        return len(nodes), np.array(edges)

    @staticmethod
    def get_graph(data: SnapData, directed: bool = False) -> Graph:
        """
        ...
        graph.add_edge_list(data[1])

        return graph

    @staticmethod
    def get_social_graph(data_file: Path = Path("data/social.txt"), separator: str = '\t') -> Graph:
        data = GraphGenerator.read_snap_txt(data_file, separator=separator)
        return GraphGenerator.get_graph(data, directed=True)

    @staticmethod
    def get_collab_graph(data_file: Path = Path("data/collab.txt"), separator: str = '\t') -> Graph:
        data = GraphGenerator.read_snap_txt(data_file, separator=separator)
        return GraphGenerator.get_graph(data, directed=False)

    @staticmethod
    def get_comm_graph(data_file: Path = Path("data/comm.txt"), separator: str = '\t') -> Graph:
        data = GraphGenerator.read_snap_txt(data_file, separator=separator)
        return GraphGenerator.get_graph(data, directed=False)
...
if __name__ == '__main__':
    ...
    def print_data(graph: Graph) -> None:
        print(f"Number of nodes: {graph.get_vertices().size}")
        print(f"Number of edges: {graph.get_edges().size}")
